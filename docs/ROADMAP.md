...
The Android beta-signing key remains **AWAITING HUMAN SIGNING KEY** —
none was generated, per this item's own explicit instruction; exact
one-time setup instructions are documented for the human owner. The
beta environment decision (reuse `forge-staging` vs. a separate
