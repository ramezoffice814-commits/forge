  ...
  group('allows', () {
    test('every type is allowed under the all-enabled defaults', () {
      ...
      for (final type in ForgeNotificationType.values) {
        expect(preferences.allows(type), isTrue, reason: '$type should be allowed by default');
      }
    });

    test('the master toggle overrides every category, even ones left enabled', () {
      const preferences = NotificationPreferences(masterEnabled: false);
      for (final type in ForgeNotificationType.values) {
        expect(preferences.allows(type), isFalse, reason: '$type must be blocked when master is off');
      }
    });

    test('disabling a specific category blocks only that category\'s type(s)', () {
      const preferences = NotificationPreferences(achievementEnabled: false);
      expect(preferences.allows(ForgeNotificationType.achievementUnlock), isFalse);
    ...
    test('competitionResultEnabled gates both week and season results', () {
      const preferences = NotificationPreferences(competitionResultEnabled: false);
      ...
      expect(preferences.allows(ForgeNotificationType.achievementUnlock), isTrue);
    });

    test('progressionEnabled gates level-up, independently of achievements', () {
